	...
	try:
		while True:
			...
			if event.type == "Idle":
				...
				print("{} Idling...".format(datetime.now()))
			elif event.type == "EpisodeStart":
				print("{} Episode Start: {}".format(datetime.now(),event.episode_start.config))
				sim.episode_start(event.episode_start.config)
				episode += 1
			elif event.type == "EpisodeStep":
				print("{} EpisodeStep".format(datetime.now()))
				iteration += 1
				sim.episode_step(event.episode_step.action)
			elif event.type == "EpisodeFinish":
				print("{} EpisodeFinish".format(datetime.now()))
				iteration = 0
			elif event.type == "Unregister":
				print("{}, Simulator Session unregistered by platform, Registering again!".format(datetime.now()))
				registered_session, sequence_id = CreateSession(registration_info, config_client)
				continue
			else:
				pass
		...
		print("{}: Unregistered simulator.".forma(datetime.now()))
	except Exception as err:
		...
		print("{}: Unregistered simulator because: {}".format(datetime.now(),err))
