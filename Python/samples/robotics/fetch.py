...
class TemplateSimulatorSession:
    ...
    def episode_start(self, config: Dict = None) -> None:
        ...
        self.state["gym_reward"] = 0
        self.state["gym_terminal"] = False
        self.state["observation"] = self.state["observation"].tolist()
        self.state["achieved_goal"] = self.state["achieved_goal"].tolist()
        self.state["desired_goal"] = self.state["desired_goal"].tolist()
    ...
    def episode_step(self, action: Dict):
        ...
        self.state["achieved_goal"] = self.state["achieved_goal"].tolist()
        self.state["desired_goal"] = self.state["desired_goal"].tolist()
        self.state["gym_reward"] = float(reward)
        self.state["gym_terminal"] = int(done)
        if self.render:
            self.env.render()
